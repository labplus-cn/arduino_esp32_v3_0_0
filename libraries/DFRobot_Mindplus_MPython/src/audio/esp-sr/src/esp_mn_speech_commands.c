...
#include "include/esp_mn_speech_commands.h"
#include "../include/esp32s3/esp_mn_iface.h"
#include "flite_g2p.h"

static const char *TAG = "MN_COMMAND";
...
esp_err_t esp_mn_commands_add(int command_id, const char *string)
{
    ...
    const char *lang = esp_mn_model_handle->get_language ? esp_mn_model_handle->get_language(esp_mn_model_data) : NULL;
    bool is_english = (lang && strcmp(lang, ESP_MN_ENGLISH) == 0);
    char *phonemes = NULL;
    char *check_target = (char *)string;

#ifdef CONFIG_SR_MN_EN_MULTINET7_QUANT
    // In mixed build environments CONFIG_SR_MN_EN_MULTINET7_QUANT may be set
    // even when runtime model is Chinese. Only force G2P for English models.
    if (is_english) {
        phonemes = flite_g2p(string, 1);
        check_target = phonemes ? phonemes : (char *)string;
    }
#endif
    if (esp_mn_model_handle->check_speech_command(esp_mn_model_data, check_target) == 0) {
        ESP_LOGE(TAG, "invalid command, please check format, %s%s%s.\n",
                 string,
                 (phonemes ? " (" : ""),
                 (phonemes ? phonemes : ""));
        if (phonemes) {
