...
mPython_WS2812::mPython_WS2812(uint16_t n /*= 8*/, uint8_t gpio /*= 2*/, uint8_t chn /*= 0*/, mPython_LED_TYPE t /*= mPython_TYPE_GRB*/)
{
	ledCounts = n;
	pin = gpio;
	rmt_chn = chn;
...
void mPython_WS2812::setLedCount(uint16_t n)
{
	ledCounts = n;
	free(led_rgb_data);
	led_rgb_data = (uint32_t *)malloc(ledCounts * sizeof(uint32_t));
...
void mPython_WS2812::setLedType(mPython_LED_TYPE t)
{
	rOffset = (t >> 4) & 0x03;
	gOffset = (t >> 2) & 0x03;
	bOffset = t & 0x03;
...
void mPython_WS2812::setBrightness(uint8_t brightness)
{
	br = constrain(brightness, 0, 255);
	show();
}

esp_err_t mPython_WS2812::setLedColorData(int index, uint8_t r, uint8_t g, uint8_t b)
{
	uint32_t color = r << 16 | g << 8 | b ;
	return setLedColorData(index, color);
}

esp_err_t mPython_WS2812::setLedColorData(int index, uint32_t rgb)
{
	led_rgb_data[index] = rgb;
	return ESP_OK;
}

esp_err_t mPython_WS2812::setLedColor(int index, uint32_t rgb)
{
	return setLedColor(index, rgb >> 16, rgb >> 8, rgb);
}

esp_err_t mPython_WS2812::setLedColor(int index, uint8_t r, uint8_t g, uint8_t b)
{
	setLedColorData(index, r, g, b);
	return show();
}

esp_err_t mPython_WS2812::setAllLedsColorData(uint32_t rgb)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t mPython_WS2812::setAllLedsColorData(uint8_t r, uint8_t g, uint8_t b)
{
	for (int i = 0; i < ledCounts; i++)
	{
...
esp_err_t mPython_WS2812::setAllLedsColor(uint32_t rgb)
{
...
esp_err_t mPython_WS2812::setAllLedsColor(uint8_t r, uint8_t g, uint8_t b)
{
...
uint16_t mPython_WS2812::getLedCount(void)
{
	return ledCounts;
}

uint8_t mPython_WS2812::getBrightness(void)
{
	return br;
}

uint32_t mPython_WS2812::getLedColorData(int index)
{
	return led_rgb_data[index];
}

uint32_t* mPython_WS2812::getLedAllColorDate(void)
{
	return led_rgb_data;
}
	
uint32_t mPython_WS2812::Wheel(byte pos)
{
...
uint32_t mPython_WS2812::hsv2rgb(uint32_t h, uint32_t s, uint32_t v)
{
	uint8_t r, g, b;
	h %= 360; // h -> [0,360]
